...
# print(samples.shape)
    ...
    #            label = label[o==1]
        ...
        if self.rep > 0:
            expr_train = np.matlib.repmat(expr, self.rep, 1)
        else:
        ...
        for e in range(self.epochs):
            ...
            if e % 100 == 0 and self.annealing:
                tau = max(self.tau0*np.exp(-anneal_rate * e), self.min_tau)
                print(tau)

            self.tau_in = np.ones(expr_train.shape, dtype='float32') * tau
            # print(tau_in.shape)
            ...
            if e % self.patience == 1:
                print("Epoch %d/%d" % (e+1, self.epochs))
                print("Loss:"+str(train_loss))
                if abs(cur_loss-prev_loss) < 1 and e > self.min_stop:
                    break
    ...
    def transform(self, data):
        return self.ae.predict([data, self.tau_in])[5]
